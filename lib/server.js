(function() {
  ...
  Follower = (function() {
    ...
    Follower.prototype.readEntries = function(cb) {
      return db.hgetall("/followers/" + this.id + "/entries", function(err, all) {
        var i, key, list, run, value;
        list = (function() {
          var _results;
          _results = [];
          for (key in all) {
        ...
        console.log("/followers/" + this.id + "/entries", list);
        i = list.length;
        return (run = function() {
          if (!i--) {
            return cb(null, list);
  ...
  Entry = (function() {
    ...
    Entry.prototype.save = function(cb) {
      ...
      op.hset("/followers/" + this.user + "/entries", this.day, this.id);
      return op.exec(__bind(function(err) {
        return cb(err, this);
      }, this));
    };
  ...
  onEntry = function(data) {
    var entry, handle, lat, lng, _ref, _ref2;
    ...
    return (new Follower({
      ...
      if (err) {
        return oa.post("http://api.twitter.com/1/statuses/update.json", TWITTER_TOKEN, TWITTER_TOKEN_SECRET, {
          ...
          return console.log(err || ("got entry from non-follower: " + handle + "."));
        });
      } else {
  ...
  onFollow = function(data) {
    ...
    return follower.save(function(err, follower) {
      ...
      return console.log("new follower: " + follower.handle);
      /*
          oa.post(
  ...
  handlers = [
    ...
    }, /^\/api\/followers\/(\w+)\/entries$/, function(req, cb) {
      return db.hget("/handles", req.captures[1], function(err, id) {
        if (!id) {
          return cb(404);
        }
