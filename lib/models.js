(function() {
  ...
  exports.User = User = (function() {
    ...
    User.prototype.attr = function() {
      return db.hset.apply(db, [this.uri].concat(__slice.call(arguments)));
    };
    User.prototype.score = function(cb) {
      var op, score;
      score = 0;
      op = db.multi();
      op.scard("" + this.uri + "/entries", function(err, num) {
        score += num || 0;
        return score += 2 * (num === 30);
      });
      op.hgetall(this.uri, function(err, props) {
        var total;
        total = !!props.practice + !!props.mention + !!props.retweet + !!props.hashtag;
        return score += total * 2;
      });
      op.hset(this.uri, "score", score);
      return op.exec(function(err) {
        return cb(err, score);
      });
    };
